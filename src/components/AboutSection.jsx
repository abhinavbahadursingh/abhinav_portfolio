...
import { Briefcase, Code, User, Download, Calendar, Sparkles, Target, Github, Linkedin, Twitter, Mail, Star, Brain, Cpu, Database, Eye } from 'lucide-react';
...
export const AboutSection = () => {
  ...
  const achievements = [
    { number: "80%", label: "Workload Cut", icon: <Cpu className="h-5 w-5" />, suffix: "" },
    { number: "30%", label: "Flow Optimization", icon: <Target className="h-5 w-5" />, suffix: "" },
    { number: "25+", label: "FPS Throughput", icon: <Eye className="h-5 w-5" />, suffix: "" },
    { number: "10+", label: "Hackathons", icon: <Briefcase className="h-5 w-5" />, suffix: "" }
  ];

  const techStack = [
    { category: "AI / ML & CV", items: ["PyTorch", "TensorFlow", "OpenCV", "YOLOv8 / YOLOv5", "FaceNet & VGG-Face", "Scikit-learn"] },
    { category: "GenAI & LLMs", items: ["LangChain", "RAG Pipelines", "LLM Fine-Tuning", "GPU Inference", "NLP & OCR", "Roboflow"] },
    { category: "Web & DevOps", items: ["Python & Java", "React.js & Next.js", "Node.js & REST APIs", "Docker & Git", "MySQL & MongoDB", "Firebase & Streamlit"] }
  ];

  const features = [
    "End-to-end ML/CV architecture",
    "Real-time video processing & RTSP streaming",
    "Generative AI & RAG pipeline engineering",
    "Full-stack React/Node.js integration",
    "Dockerized GPU-accelerated inference",
    "Agile SDLC & CI/CD workflows"
  ];

  const socialLinks = [
    { icon: <Github className="h-5 w-5" />, href: "https://github.com/abhinavbahadursingh" },
    { icon: <Linkedin className="h-5 w-5" />, href: "https://linkedin.com/in/abhinavbahadursingh" },
    { icon: <Mail className="h-5 w-5" />, href: "mailto:abhinavbahadursingh69@gmail.com" }
  ];

  const tabContent = {
    personal: "AI/ML and Computer Vision engineer passionate about solving complex real-world challenges through intelligent systems. Currently pursuing B.Tech in CSE at AKTU (2027), constantly prototyping in hackathons and contributing to open source.",
    professional: "Proven track record delivering high-impact automation—from on-site industrial safety monitoring at NTPC Unchahar to biometric systems at ABESIT NVIDIA Deep Learning Centre of Excellence, reducing manual administrative workload by 80%.",
    approach: "I focus on production-grade machine learning: rigorous dataset curation, model fine-tuning (YOLOv8/LLMs), real-time low-latency optimization (Kalman filters, GPU inference), and clean full-stack deployment."
  };
  ...
  const handleDownload = () => {
    ...
    link.download = 'Abhinav_Bahadur_Singh_Resume.pdf';
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl overflow-hidden border-4 border-primary/20 shadow-2xl transition-all duration-500 group-hover:border-primary/40 group-hover:scale-105 md:group-hover:scale-110 relative">
                      <img src="/profile.png" alt="Abhinav Bahadur Singh" className="w-full h-full object-cover" />
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h2 className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2">Abhinav Bahadur Singh</h2>
                    <p className="text-primary text-base sm:text-lg font-semibold mb-3 sm:mb-4">AI/ML & Computer Vision Engineer</p>
