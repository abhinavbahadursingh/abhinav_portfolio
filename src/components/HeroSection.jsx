import { ArrowDown, MousePointerClick, Sparkles, Code, Palette, Rocket, Award, Download, Calendar, Shield, Zap, Users, TrendingUp, Briefcase, Mail, Brain, Cpu, Eye, Activity } from "lucide-react";
...
export const HeroSection = () => {
  ...
  const codeSnippets = [
    ...
    "class AIMLEngineer:",
    "    def __init__(self):",
    "        self.name = 'Abhinav Bahadur Singh'",
    "        self.focus = ['Computer Vision', 'GenAI RAG', 'Deep Learning']",
    "        self.stack = ['PyTorch', 'OpenCV', 'YOLOv8', 'LangChain', 'React.js']",
    "        self.models = ['PPE-Compliance-YOLO', 'Smart-Traffic-SUMO', 'Biometrics-FaceNet']",
    "",
    "    def deploy_intelligence(self):",
    "        # 80% administrative automation & 30% latency optimization",
    "        return '🚀 Building production-grade AI pipelines & real-time CV solutions'",
    "",
    "engineer = AIMLEngineer()",
    "print(engineer.deploy_intelligence())"
  ];

  const achievements = [
    { number: "80%", label: "Workload Automation", icon: <Cpu className="h-3 w-3" /> },
    { number: "30%", label: "Latency Optimized", icon: <TrendingUp className="h-3 w-3" /> },
    { number: "25+ FPS", label: "Real-Time CV", icon: <Eye className="h-3 w-3" /> },
    { number: "10+", label: "Hackathons", icon: <Award className="h-3 w-3" /> }
  ];
        ...
        <motion.div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.25, delayChildren: 0.5 } } }}>
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <span className="block text-foreground">I'm Abhinav</span>
            ...
            <motion.p className="text-lg sm:text-xl text-muted-foreground mt-6 leading-relaxed max-w-2xl" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              Specialized in architecting <span className="text-primary font-semibold">end-to-end intelligent systems</span> ranging from real-time biometric recognition to Generative AI RAG pipelines. Delivering high-impact automation with deep learning and full-stack integration.
            </motion.p>
            ...
            <motion.div className="mt-6 text-center lg:text-left" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <div className="text-sm text-muted-foreground">
                🚀 <span className="text-primary font-semibold">Open to Opportunities</span> in AI/ML, Computer Vision & Full-Stack roles
              </div>
            </motion.div>
          </div>
